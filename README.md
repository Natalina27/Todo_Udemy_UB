...
> 22. Search release 
>> Компонент App  получил новый элемент state,
     в котором мы храним текст для поиска
>> Перед тем как отображать элементы в render() мы отфильтровываем нужные
>> Компонент SeachPanel генерирует событие onSearchChange на каждое нажатие клавиши
